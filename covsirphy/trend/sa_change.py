...
class _SAChange(Term):
    ...
    def __init__(self, sr_df, aggregate_cols = ["Isolated", "Recovered", "Dead"]):
        self.R = "Aggregate"
        self._ensure_dataframe(sr_df, name="sr_df", time_index=True, columns=aggregate_cols)
        # Index: Date, Columns: Recovered, Susceptible, logS
        self._sr_df = pd.DataFrame(
            {
                self.R: sr_df[aggregate_cols].sum(axis=1),
                self.S: sr_df[self.S],
                "logS": np.log(sr_df[self.S].astype(np.float64)),
            }
        )
    ...
    def _fitting_in_phase(self, start, end):
        ...
        return pd.DataFrame({self.FITTED: np.exp(f_partial(df[self.R]))}, index=df.index)

    def _fitting(self, change_points):
        """
        Perform curve fitting with the actual values on S-A plane.
    ...
    def show(self, change_points, area, **kwargs):
        ...
        # Title
        if change_points:
            title = f"{area}: phases detected with S-A trend analysis"
        else:
            title = f"{area}: S-A trend without change points"
        # Curve fitting
        fit_df = self._fitting(change_points)
        # Show S-R plane
