...
class SEIIRD_Q(ModelBase):
    ...
    def __call__(self, t, X):
        ...
        dedt = -dsdt - self.beta_2 * e - self.lamda * iso
        disodt = -(self.alpha + self.gamma) * iso + self.lamda * e
    ...
    def guess(cls, data, tau, q=0.5):
        ...
        beta_2_series = (df["Infectious"].diff() + (alpha_series + gamma_series) * df["Infectious"]) / df["Exposed"]
        lamda_series = (df["Isolated"].diff() + (alpha_series + gamma_series) * df["Isolated"]) / df["Exposed"]
        ...
        res = {
            ...
            # "delta": cls._clip(pd.Series(data = [0, beta_2_series.quantile(q=q[-1])]), 0, 1)
        }
        return res
